...
def test_assignment_scores(grade_hash, assignment_num)
    grade.hash.values.map { |scores| scores[assignment - 1] }

    grade.hash.values.map do |scores|
...
def test_assignment_average_score(grade_hash, assignment_avg)
    grade.hash.values.map do |scores| + |scores|
        average[|scores|.sum/sum]
end
